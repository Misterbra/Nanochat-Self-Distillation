...
def sample_next_token(logits, rng, temperature=1.0, top_k=None, top_p=None):
    ...
    # Apply top-k truncation: keep only top k logits, mask out the rest
    if top_k is not None and top_k > 0:
        k = min(top_k, logits.size(-1))
        top_vals, _ = torch.topk(logits, k, dim=-1)
        min_top_val = top_vals[:, -1:] # (B, 1) - the k-th largest value
        logits = logits.where(logits >= min_top_val, torch.full_like(logits, float('-inf')))
    # Apply top-p (nucleus) truncation: keep smallest set of tokens with cumulative prob >= top_p
